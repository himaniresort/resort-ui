...
export default function RoomTypeDialog({
  ...
  const facilities =
    roomType?.type === "tent" ? TENT_FACILITIES : DELUXE_FACILITIES;
  ...
  return (
    ...
    >
      <DialogContent dividers sx={{ ...hideScrollBar, p: isMobile ? 0 : 1 }}>
        <Grid container spacing={1} sx={{ p: isMobile ? 0 : 3 }}>
          ...
          <Grid
            ...
            xs={12}
            md={5}
            sx={{ backgroundColor: isMobile ? "#ededed" : "" }}
          >
            <Box sx={{ display: "flex", justifyContent: "space-between" }}>
              <DialogTitle
                sx={{ fontWeight: "bold", px: isMobile ? 2 : 3, pt: 0 }}
              >
                {roomType?.name}
              </DialogTitle>
              {!isMobile && (
            ...
            <Box sx={{ px: isMobile ? 2 : 3 }}>
              ...
              >
                <Box sx={{ display: "flex", gap: "4px" }}>
                  <PeopleOutlinedIcon
                    sx={{ fontSize: 20, position: "relative", top: "0px" }}
                  />
                  <Typography variant="subtitle2">
                    {roomType?.max_occupancy} {ROOM_TYPE_CONSTANTS.GUESTS}
                  </Typography>
                </Box>
                {roomType && roomType.type === "deluxe" && (
                  <Box sx={{ display: "flex", gap: "4px" }}>
                    ...
                    <Typography variant="subtitle2">
                      {ROOM_TYPE_CONSTANTS.SWIMMING_POOL_VIEW}
                    </Typography>
                  </Box>
                )}
                <Box sx={{ display: "flex", gap: "4px" }}>
                  <BathtubOutlinedIcon
                    sx={{ fontSize: 18, position: "relative" }}
                  />
                  <Typography variant="subtitle2">{`${
                    roomType?.type === "deluxe" ? 2 : ""
                  } ${
                    roomType?.type !== "tent" ? "Private" : "Common"
                  } bathroom`}</Typography>
                </Box>
                <Box sx={{ display: "flex", gap: "4px" }}>
                  <WifiOutlinedIcon
              ...
              <Box mb={2}>
                <Box sx={{ display: "flex", gap: "4px" }}>
                  <BathtubOutlinedIcon
                    sx={{ fontSize: 20, position: "relative", top: "2px" }}
                  />
              ...
              <Box mb={2}>
                <Box sx={{ display: "flex", gap: "4px" }}>
                  ...
                  <Typography variant="subtitle1" fontWeight={"bold"}>
                    {" "}
                    {ROOM_TYPE_CONSTANTS.FACILITIES}
                  </Typography>
                </Box>
                <Box>
                  <List
              ...
              <Box sx={{ display: "flex", gap: "4px", mb: isMobile ? 2 : 0 }}>
                ...
                <Box display={"flex"}>
                  ...
                  <Typography variant="body2" p={"4px"}>
                    {ROOM_TYPE_CONSTANTS.NO_SMOKING}
                  </Typography>
                </Box>
              </Box>
            </Box>
      ...
      {isMobile && (
        <DialogActions>
          <Button onClick={() => setOpenDialog(false)} sx={dialogButtonStyle}>
